    ...
    '8. Add 1 cup of tomato puree and simmer for 5-7 minutes.',
     '9. Add the grilled chicken and 1/2 cup of cream. Simmer for 3-5 minutes.',
        ...
        'Broccoli Cheddar Soup': [
            ...
            '3. Stir in 1/4 cup of all-purpose flour until smooth.',
            '4. Gradually whisk in 4 cups of chicken broth until there are no lumps.',
            '5. Add 4 cups of broccoli florets and 1 teaspoon of salt. Bring to a boil, then reduce heat and simmer until broccoli is tender, about 15-20 minutes.',
            '6. Stir in 2 cups of shredded cheddar cheese until melted.',
            '7. Stir in 1 cup of milk or cream.',
...
const getInstructions = async (dish: string | undefined) => {
  if (!dish) {
    return null;
  }
  return instructionsData[dish] || null;
};

export default async function InstructionsPage({searchParams}: InstructionsPageProps) {
  const dish = searchParams?.dish;
  const instructions = await getInstructions(dish);
      ...
      <div className="mb-4 p-4 border rounded-md shadow-sm">
        {instructions ? (
          instructions.length > 0 ? (
            ...
            <p>No specific instructions found for this dish. Here are some generic cooking tips:</p>
          )
        ) : (
          <p>No instructions found for this dish.</p>
        )}
      </div>
